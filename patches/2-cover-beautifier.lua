...
local function patchCoverBrowser(plugin)
    ...
    local settings = {
        ...
        round_corners = BooleanSetting(_("Round cover corners"), "folder_round_corners", true)
    }
    ...
    function MosaicMenuItem:update(...)
        ...
        if settings.round_corners.get() then
            -- remove the description marker
            self.has_description = false

            -- remove dogear reading indicator
            self.do_hint_opened = false

            -- round corners of covers
            local cover_container = self._underline_container[1]
            cover_container[1].bordersize = Size.border.thick
            cover_container[1].width = cover_container[1][1]:getSize().w + 2*Size.border.thick
            cover_container[1].height = cover_container[1][1]:getSize().h + 2*Size.border.thick
            cover_container[1].radius = Screen:scaleBySize(10)
            cover_container[1].background = Blitbuffer.COLOR_WHITE
        end
    ...
    function MosaicMenuItem:_setFolderCover(img)
        local top_h = settings.round_corners.get() and 0 or 2 * (Folder.edge.thick + Folder.edge.margin)
        local target = {
        ...
        -- rounded corners
        if settings.round_corners.get() == true then
            image_widget.background = Blitbuffer.COLOR_WHITE
            image_widget.radius = radius
        end
        ...
        -- Conditionally add the LineWidgets if round_corners is false
        if settings.round_corners.get() == false then
            table.insert(folder_widgets, 2, LineWidget:new {
                background = Folder.edge.color,
                dimen = { w = math.floor(dimen.w * (Folder.edge.width ^ 2)), h = Folder.edge.thick },
            })
            table.insert(folder_widgets, 4, LineWidget:new {
                background = Folder.edge.color,
        ...
        local widget = CenterContainer:new {
            ...
            VerticalGroup:new(folder_widgets),
        }
